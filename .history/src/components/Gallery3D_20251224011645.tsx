...
function resolveDriveUrl(url: string) {
  try {
    ...
    if (u.hostname.includes("drive.google.com")) {
      ...
      if (id && id.trim()) {
        id = id.trim();
        // Primary method: Google Direct Access (works for most cases)
        // This URL format serves images directly without confirmation page
        const resolved = `https://drive.google.com/uc?export=view&id=${id}&access_token=nope&confirm=t`;
  ...
  const texture = useMemo(() => {
    try {
      if (event.type === "image") {
        ...
        const resolved = localPreview ? localPreview : resolveDriveUrl(event.url);
          ...
          <div className="p-4 border-t border-slate-200 bg-white">
            ...
            <div className="flex items-center gap-3 overflow-x-auto pb-2">
              {events.map((ev) => {
                ...
                const thumbUrl = localPreview ? localPreview : resolveDriveUrl(ev.url);
                return (
                  <button
                    key={ev.id}
