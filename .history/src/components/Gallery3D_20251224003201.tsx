...
function Earth() {
  ...
  return (
    <Sphere ref={ref} args={[1.5, 64, 64]}>
      <meshStandardMaterial wireframe transparent opacity={0.03} color="#3b82f6" />
    </Sphere>
  );
}
...
function CurvedPhoto({
  ...
  const radius = 1.65;
  // slightly increase radius for better spacing and visual balance
  const radiusAdjusted = radius + 0.1;
  ...
  useFrame((_, delta) => {
    ...
    // delta-based smoothing to avoid snapping across frame rates
    const lerpFactor = 1 - Math.exp(-8 * delta);
    groupRef.current.scale.lerp(new THREE.Vector3(s, s, s), lerpFactor);
    // gentle facing towards camera: make the tile orient slightly outward
    const lookAt = new THREE.Vector3(x, y, z).multiplyScalar(1.05);
    groupRef.current.quaternion.slerp(new THREE.Quaternion().setFromRotationMatrix(new THREE.Matrix4().lookAt(groupRef.current.position, new THREE.Vector3(0, 0, 0), new THREE.Vector3(0, 1, 0))), 0.06);
  });
          ...
          {events.length === 0 ? (
            ...
            <Canvas dpr={[1, 1.5]} camera={{ position: [0, 0, 6], fov: 50 }}>
              <Suspense fallback={null}>
                {/* Soft hemisphere for natural fill */}
                <hemisphereLight skyColor={0xffffff} groundColor={0x444b6a} intensity={0.6} />
                {/* Subtle directional key light for readable images */}
                <directionalLight position={[5, 6, 2]} intensity={0.7} castShadow={false} />
                {/* Gentle warm rim light to separate tiles from background */}
                <pointLight position={[-6, -4, -2]} intensity={0.14} color={0xfff4e6} />
