      ...
      <Card className="max-w-6xl mx-auto">
        <CardContent className="p-0 h-[700px] relative bg-white">
          {events.length === 0 ? (
            ...
            <Canvas dpr={[1, 1.5]} camera={{ position: [0, 0.3, 6.5], fov: 48 }} gl={{ antialias: true }}>
              <Suspense fallback={null}>
                {/* Balanced hemisphere for even natural fill */}
                <hemisphereLight skyColor={0xfafbfc} groundColor={0x5a6a7a} intensity={0.5} />
                {/* Warm key light for readable, vibrant images */}
                <directionalLight position={[4, 7, 3]} intensity={0.85} color={0xfaf6f1} castShadow={false} />
                {/* Subtle cool fill light to balance warm key */}
                <directionalLight position={[-3, 2, -4]} intensity={0.3} color={0xe8f0ff} castShadow={false} />
